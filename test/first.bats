...
@test "date-dedash accepts multiple files" {
    cd $BATS_TMPDIR
    touch abc def ghi
    output="$($CMD abc def ghi | grep -ic processing)"
    [ "$output" -eq 3 ]
    rm abc def ghi
}
...
@test "date-dedash handles filenames properly with spaces" {
    ...
    run $CMD "2019-03-19 File With Spaces1"
    ...
    run $CMD 2019-03-19\ File\ With\ Spaces2
    ...
    run $CMD "2019-03-20 File With Spaces1" "2019-03-20 File With Spaces2" "2019-03-20 File With Spaces3"
